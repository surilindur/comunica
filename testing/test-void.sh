...
#COMUNICA_CONFIG="$COMUNICA_CONFIG" node --max-old-space-size=16384 "$COMUNICA_BIN" https://sparql.rhea-db.org/sparql --file "$QUERYX" --showStackTrace

#COMUNICA_CONFIG="$COMUNICA_CONFIG" node --max-old-space-size=16384 "$COMUNICA_BIN" https://sparql.uniprot.org/sparql https://sparql.rhea-db.org/sparql --file "$QUERY1" --showStackTrace
...
COMUNICA_CONFIG="$COMUNICA_CONFIG" node "$COMUNICA_BIN" https://query.wikidata.org/sparql https://sparql.rhea-db.org/sparql --file "$QUERY3" --showStackTrace -l debug
