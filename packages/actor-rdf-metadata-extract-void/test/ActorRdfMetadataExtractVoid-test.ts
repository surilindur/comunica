...
describe('ActorRdfMetadataExtractVoid', () => {
  ...
  describe('getTriples', () => {
    it('should return triple count when present', async() => {
      jest.spyOn((<any>actor).queryEngine, 'queryBindings').mockResolvedValue(streamifyArray([
        { get: () => ({ value: '123' }) },
      ]));
      await expect(actor.getTriples(<any>{}, <any>{})).resolves.toBe(123);
    });

    it('should return 0 when no triple count is present', async() => {
      jest.spyOn((<any>actor).queryEngine, 'queryBindings').mockResolvedValue(streamifyArray([]));
      await expect(actor.getTriples(<any>{}, <any>{})).resolves.toBe(0);
    });
  });
});
