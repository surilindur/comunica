  ...
  public async run(action: IActionHttp): Promise<IActorHttpOutput> {
    ...
    while (attempt < retryCount) {
      ...
      if (response.status === 429 || response.status === 503) {
        ...
        const retryAfter = this.parseRetryAfterHeader(response.headers.get('retry-after') ?? retryDelay.toString(10));
      ...
      this.logDebug(action.context, 'Dereferencing failed', () => ({
        url: url.href,
        status: response.status,
        statusText: response.statusText,
        attemptCount: attempt,
    ...
    throw new Error(`Dereferencing failed after ${attempt} attempts: ${url.href}`);
  }

  public async waitUntil(date: Date): Promise<void> {
    const delay = Math.max(0, date.getTime() - Date.now());
    await new Promise(resolve => setTimeout(resolve, delay));
  }

  public parseRetryAfterHeader(retryAfter: string): Date {
