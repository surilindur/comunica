...
export class HttpServiceSparqlEndpoint {
  ...
  public async handleRequest(
    ...
    try {
      ...
      process.stdout.write(`Worker ${process.pid} processing request for ${operation.type}\n`);

      if (operation.queryString) {
        process.stdout.write(`Worker ${process.pid} executing query:\n${operation.queryString}\n`);
      }
      ...
      // When the response stream starts, assign the status code and content type
      response.statusCode = 200;
      response.setHeader('Content-Type', mediaType);

      await new Promise<void>((resolve, reject) => {
        data.on('error', reject).on('end', resolve).pipe(response);
      });

      stdout.write(`Worker ${process.pid} responded with ${result.resultType} as ${mediaType}\n`);
    } catch (error: unknown) {
  ...
  public async runWorker(stdout: Writable, stderr: Writable): Promise<void> {
    ...
    const terminateWorker = async(code: number): Promise<void> => {
      server.close(() => {
        stdout.write(`Worker ${process.pid} closed its server successfully\n`);
        // eslint-disable-next-line unicorn/no-process-exit
        process.exit(code);
      });
      // After all the pending responses are closed, the above close callback will fire
      await Promise.all([ ...openResponses.values() ].map(connection => new Promise<void>((resolve) => {
        if (code === 15) {
          connection.end(resolve);
        } else {
          connection.end('!TERMINATED!', 'utf-8', resolve);
        }
      })));
    };
    ...
    const server = createServer((request: IncomingMessage, response: ServerResponse) => {
      ...
      const onResponseDone = (): void => {
        ...
        if (openResponses.delete(response) && this.freshWorkerPerQuery && request.method !== 'HEAD') {
          terminateWorker(15).catch(printError);
        }
      };
      // Register termination on both finish and error
      response.on('finish', onResponseDone).on('error', onResponseDone);
    ...
    process.on('message', (message: string) => {
      switch (message) {
        case 'terminate':
          terminateWorker(70).catch(printError);
          break;
        default:
